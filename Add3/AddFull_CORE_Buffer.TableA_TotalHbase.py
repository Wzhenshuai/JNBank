...
for ta in all_table:
    ...
    file_sql_name = "AddFull_CORE_Buffer.%s_TotalHbase.sql" % (table_name)
    # 查询表的注解
    ...
    create_table_str = create_table_str+unite_key_file.rstrip(",")+"拼接)',\r" \
                                          "DataDay_ID String COMMENT '数据的时间',\r" \
                                          "tdh_load_timestamp  String  COMMENT'加载到TDH时的时间戳',\r"
    ...
    create_table_str = create_table_str+"Data_source String COMMENT'数据来源'\r)comment '%s汉语注解.贴源层全量表' STORED BY 'org.apache.hadoop.hive.hbase.HBaseStorageHandler';"%(table_name)

    #insert_table_str = "\rfrom AddAnticipate.%s;"%(table_name)
