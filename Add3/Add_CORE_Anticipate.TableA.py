...
for ta in all_table:
    ...
    exe_table_name = (ta[0])
    file_sql_name = "Add_CORE_Anticipate.%s.sql" % (table_name)
    # 查询表的注解
    ...
    create_table_str = create_table_str+"Data_source String COMMENT'数据来源'\r)comment '%s汉语注解' STORED AS TEXTFILE TBLPROPERTIES('serialization.null.format'='');"%(table_name)
