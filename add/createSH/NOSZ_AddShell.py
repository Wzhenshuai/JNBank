...
cursor.execute(
    "SELECT en_name,system_en_name FROM table_scheme WHERE system_name = '%s' AND or_extract = '是' and provideDate_way = '增量'" %system_na)
table_datas = cursor.fetchall()

dictSql = "SELECT shell_path,system_code FROM dic_info_mapping WHERE transfer_mode ='增量数据' AND system_code='" + systemUpper + "'"
...
allTempFilePath = r"E:\mnt\template\add\Core_agentShell\AddData.Core.SHORTNAME_tablename.sh"
...
for td in table_datas:
    ...
    for fd in field_datas:
        ...
        if fd[0].upper() == 'CORPORATION':
            field1 = "select "
    fieldStr = field1 + fieldStr.rstrip(',')+" from ${source_Table} where DAY_ID = '${%s}'and \$CONDITIONS"%tableDate
