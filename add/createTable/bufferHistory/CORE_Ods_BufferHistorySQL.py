...
out_file_path = os.path.join(path, "CORE.Core.%s.AddBufferHisAdd.sql" % system_st)
...
for td in table_datas:
    ...
    drop_table_str = 'DROP TABLE IF EXISTS ' + COREtablename + '_HisAdd;\r'
    fieldStr += "`Data_source_str` varchar(33) COMMENT'数据来源'"
    create_str = drop_table_str + 'create table IF NOT EXISTS ' + COREtablename + ' (_HisAdd\r' + fieldStr.rstrip(',\r') + "\r)comment '%s' partitioned by(partition_day varchar(33)) \r clustered by (rowKeyStr) into 6 buckets stored as orc TBLPROPERTIES ('transactional'='true');;\r\r\r"%tableName.upper()
