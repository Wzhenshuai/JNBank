...
cursor.execute(
    "SELECT system_en_name,en_name,ch_name FROM table_scheme WHERE system_name ='%s' AND or_extract='是' and core_town = '1'" % system_nu)
...
def exec():
    ...
    for td in table_datas:
        ...
        boday = "DROP TABLE IF EXISTS Town_%s; \r create external table IF NOT EXISTS Town_%s(\r" % (tableName, tableName)
