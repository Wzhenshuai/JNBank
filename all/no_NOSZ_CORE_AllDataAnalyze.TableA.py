...
for ta in allTable:
    ...
    cursor.execute("SELECT field_code,field_type,field_len,field_accuracy,field_name,key_flag "
                   "FROM table_field where scheme_key ='%s' order by field_id" % (ta[0]))
    allField = cursor.fetchall()
    catalog = shortName + "_" + tableName.lower()
    table_name = "Core_"+ catalog
    file_sql_name = "AllDataAnalyze.%s.sql" % ("Core." + catalog)
    exe_table_name = (ta[0])
    ...
    for i in allField:
        ...
        if i[0] == 'corporation':
            ii = 1
        fieldStr = fieldStr+("%s string comment '%s',\n")%('`'+i[0]+'`',comm)
    if ii == 0:
        create_table_str = create_table_str + "`corporation` String comment '法人行号_主鍵',\n" + fieldStr
    else:
        create_table_str = create_table_str + fieldStr
    create_table_str = create_table_str.rstrip(",\n")+"\r)comment '%s汉语注解' row format delimited fields terminated by '\\u0003' \r" \
                                                      "stored as textfile location '/DATACENTER/AllData/CREDIT/CoreBank/%s/';" % (table_name,catalog)
    ## 插入语句拼接
